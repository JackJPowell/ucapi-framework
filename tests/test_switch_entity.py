"""Tests for SwitchEntity with built-in state management."""
...
class TestSwitchEntity:
    """Test SwitchEntity state management."""
    ...
    def mock_api(self):
        ...
        api.configured_entities.get.return_value = MagicMock(
            attributes={switch.Attributes.STATE: switch.States.OFF}
        )
        return api

    @pytest.fixture
    def entity(self, mock_api):
        """Create a SwitchEntity for testing."""
        entity = SwitchEntity(
            "switch.test",
            "Test Switch",
            features=[switch.Features.ON_OFF],
            attributes={switch.Attributes.STATE: switch.States.OFF},
        )
    ...
    def test_initial_state(self, entity):
        ...
        assert entity.state == switch.States.OFF

    def test_set_state_on_with_update(self, entity, mock_api):
        """Test set_state(ON) calls entity.update() by default."""
        entity.set_state(switch.States.ON, update=True)

        assert entity.state == switch.States.ON
        ...
        assert entity_id == "switch.test"
        assert switch.Attributes.STATE in attributes
        assert attributes[switch.Attributes.STATE] == switch.States.ON
    ...
    def test_set_state_off_with_update(self, entity, mock_api):
        ...
        mock_api.configured_entities.get.return_value = MagicMock(
            attributes={switch.Attributes.STATE: switch.States.ON}
        )
        entity.set_state(switch.States.OFF, update=True)
    ...
    def test_set_state_without_update(self, entity, mock_api):
        ...
        entity.set_state(switch.States.ON, update=False)
    ...
    def test_set_state_unavailable(self, entity, mock_api):
        ...
        entity.set_state(switch.States.UNAVAILABLE, update=False)
        assert entity.state == switch.States.UNAVAILABLE
    ...
    def test_set_state_none_does_not_send_none(self, entity, mock_api):
        ...
        if mock_api.configured_entities.update_attributes.called:
            ...
            assert switch.Attributes.STATE not in attributes

    def test_property_getter_is_read_only(self, entity):
        """Test that the state property cannot be set directly."""
        with pytest.raises(AttributeError):
            entity.state = switch.States.ON  # type: ignore[misc]

    def test_only_one_attribute(self, entity, mock_api):
        """Test that SwitchEntity only manages the STATE attribute."""
        entity.set_state(switch.States.ON, update=True)

        call_args = mock_api.configured_entities.update_attributes.call_args
        _, attributes = call_args[0]
        assert list(attributes.keys()) == [switch.Attributes.STATE]

    def test_no_set_attributes_method(self, entity):
        """Test that SwitchEntity does not have a set_attributes bulk helper."""
        assert not hasattr(entity, "set_attributes")


class TestSwitchEntityInheritance:
    """Test that SwitchEntity can be subclassed and overridden."""
